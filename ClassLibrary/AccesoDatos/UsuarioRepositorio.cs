...
namespace ClassLibrary.Datos
{
    public class UsuarioRepositorio : IUsuarioRepositorio
    {
        ...
        public void ActualizarDatosCliente(int usuarioId, string nombre, Pais pais)
        {
            string sql = @"UPDATE Usuarios SET Nombre = @Nombre, PaisId = @Pais WHERE Id = @UsuarioId";

            using (SqlConnection conexion = Conexion.ObtenerConexion())
            using (SqlCommand comando = new SqlCommand(sql, conexion))
            {
                comando.Parameters.Add("@Nombre", SqlDbType.NVarChar, 150).Value = nombre;
                comando.Parameters.Add("@Pais", SqlDbType.TinyInt).Value = (byte)pais;
        ...
        public void ActualizarDatosShaper(
            int usuarioId, string nombre, Pais pais, string nombreDeNegosio, string contacto, string logoUrl)
        {
            string sql = @"
        UPDATE Usuarios
        SET Nombre = @Nombre,
        ...
        WHERE Id = @UsuarioId";
            ...
            using (SqlCommand comando = new SqlCommand(sql, conexion))
            {
                comando.Parameters.Add("@Nombre", SqlDbType.NVarChar, 150).Value = nombre;
                comando.Parameters.Add("@Pais", SqlDbType.TinyInt).Value = (byte)pais;
                comando.Parameters.Add("@NombreDeNegosio", SqlDbType.NVarChar, 150).Value = nombreDeNegosio;
                comando.Parameters.Add("@Contacto", SqlDbType.NVarChar, 150).Value = contacto;
                comando.Parameters.Add("@LogoUrl", SqlDbType.NVarChar, 500).Value = (object)logoUrl ?? DBNull.Value;
                comando.Parameters.Add("@UsuarioId", SqlDbType.Int).Value = usuarioId;
                conexion.Open();
                comando.ExecuteNonQuery();
            }
