...
document.addEventListener('DOMContentLoaded', () => {
  updateBadge();
    ...
    function openModal(key) {
      ...
      const boardsHTML = s.boards.map(b => `
        <div class="modal-board-item">
          <p class="modal-board-name">${b.name}</p>
          <p class="modal-board-spec">${b.spec}</p>
          <p class="modal-board-price">${b.price}</p>
          ${b.id ? `<button onclick="addBoard(${b.id})" style="margin-top:.5rem;width:100%;background:var(--ocean);color:#fff;border:none;padding:.4rem;font-size:.7rem;letter-spacing:.07em;text-transform:uppercase;border-radius:2px;cursor:pointer;transition:background .2s;" onmouseover="this.style.background='var(--wave)'" onmouseout="this.style.background='var(--ocean)'">+ Agregar</button>` : ''}
        </div>
      `).join('');
      ...
      document.getElementById('modalPanel').innerHTML = `
        <div class="modal-hero" style="background:${s.color}">
          <svg viewBox="0 0 820 260" xmlns="http://www.w3.org/2000/svg">
            <rect width="820" height="260" fill="${s.color}"/>
            <ellipse cx="180" cy="110" rx="90" ry="95" fill="${s.accent}"/>
            <rect x="90" y="185" width="180" height="200" rx="14" fill="${s.accent}"/>
            <ellipse cx="660" cy="130" rx="40" ry="140" fill="rgba(67,148,224,.08)" transform="rotate(-15 660 130)"/>
            <ellipse cx="720" cy="80" rx="30" ry="100" fill="rgba(67,148,224,.05)" transform="rotate(-20 720 80)"/>
        ...
        <div class="modal-body">
          ...
          <div class="modal-boards-grid">${boardsHTML}</div>
          <div class="modal-cta">
